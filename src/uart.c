...
static volatile unsigned char UART_Newlines;
...
//    if( (data == 10)||(data==13) ) /* one less newline in buffer */
    if( data==13 ) /* \r */
    	UART_Newlines--;
...
/*************************************************************************
Function: uart_have_newline() 
Purpose:  Determine the number of newline symbols waiting in the receive buffer
Input:    None
Returns:  Integer number of newline symbols in the receive buffer
**************************************************************************/
int uart_have_newline(void)
{
	if(!uart_available())/* just a precaution, not actually needed (TM) */
		return 0;

	return UART_Newlines;
} /* uart_have_newline */
#endif /* ATMEGA_USART0 */
