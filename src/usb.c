 ...
 * usb.c                                                                        *
 *                                                                              *
 * Author: Krishna Mani (https://github.com/kmani314)                           *
 *         Henryk Richter                                                       *
 *                                                                              *
 *                                                                              *
 * Purpose: USB Device interface for USB-equipped 8 bit AVRs                    *
 *          Keyboard HID                                                        *
 *                                                                              *
 *                                                                              *
 *                                                                              *
...
volatile uint8_t keyboard_pressed_keys[ USB_KB_NKEYS ] = {0, 0, 0, 0, 0, 0};
...
uint8_t usb_config_status = 0;
uint8_t keyboard_protocol; // This doesn't matter at all, we just need it for supporting a request

volatile uint8_t keyboard_leds;  // You don't need to use this if you don't want
...
*/
int usb_init() {
//  cli();  // Global Interrupt Disable (done in main() )
...
//  PLLCSR |= 0x12;  // Configure to use 16mHz oscillator
  ...
  usb_config_status = 0;
//  sei();  // Global Interrupt Enable (done in main() )
  return 0;
}

/*
  send the list of active keys
*/
int usb_send() {
  uint8_t i;
    ...
    if (keyboard_idle_value &&
      ...
      if (UEINTX & (1 << RWAL)) {  // Check if banks are writable
        ...
        if (current_idle ==
          ...
          UEDATX = 0;
          for ( i = 0; i < 6; i++) {
            UEDATX = 0;//keyboard_pressed_keys[i];
          }
          UEINTX = 0b00111010;
        }
      }
