...
void UpdateLEDButton( struct myWindow *win, ULONG code, struct Gadget *gad, struct myGadProto *prot )
{
	...
	win->active_led = ledidx;
	 ...
	 GT_SetGadgetAttrs( win->CycleSrc, win->window, NULL, GTCY_Active,map);
...
void UpdateSrcState( struct myWindow *win, ULONG code, struct Gadget *gad  )
{
	...
	if( code & MX_FORCE_NEW )
	{
		...
		GT_SetGadgetAttrs( gad, win->window, NULL, GTMX_Active, code );
		return;
	}
	...
	if( gad == win->SrcState )
	{
		ULONG val = TRUE;
		if( code == 0 ) /* off state */
			GT_SetGadgetAttrs( win->CycleSrc, win->window, NULL, GTCY_Active,0);
		else
			val = FALSE;
					
		GT_SetGadgetAttrs( win->CycleSrc, win->window, NULL, GA_Disabled,val);
...
void UpdateSliders( struct myWindow *win, ULONG code, struct Gadget *gad, struct TagItem *tlist, LONG color24 )
{
 ...
 else
 {
    ...
    GT_GetGadgetAttrs( win->sliderR, win->window, NULL, GTSL_Level, (ULONG)&sval );
    rgb.cw_Red   = UMult32( sval, 0x01010101 );
    GT_GetGadgetAttrs( win->sliderG, win->window, NULL, GTSL_Level, (ULONG)&sval );
    rgb.cw_Green = UMult32( sval, 0x01010101 );
    GT_GetGadgetAttrs( win->sliderB, win->window, NULL, GTSL_Level, (ULONG)&sval );
 ...
 if( gad != win->sliderR )
 	GT_SetGadgetAttrs( win->sliderR, win->window, NULL, GTSL_Level, rgb.cw_Red>>24 );
 if( gad != win->sliderG )
 	GT_SetGadgetAttrs( win->sliderG, win->window, NULL, GTSL_Level, rgb.cw_Green>>24 );
 if( gad != win->sliderB )
 	GT_SetGadgetAttrs( win->sliderB, win->window, NULL, GTSL_Level, rgb.cw_Blue>>24 );
...
//		Printf("Class %ld Code %ld Gad %lx\n",class,code,(ULONG)gad);
			...
			case IDCMP_INTUITICKS:
					...
					if( win->IdleTickCount > 5 )
					{
						...
						res1 = ledmanager_sendConfig(-1);
						if( res1 & KCMD_NOWORK )
							win->IdleTickCount = 0;
						else	win->IdleTickCount = 2;
